...
type StudentTab = 'today' | 'growth' | 'rewards' | 'report';
type RewardFilter = 'all' | 'didi' | 'family';

const studentNav: { key: StudentTab; label: string; icon: typeof Home }[] = [
...
export default function StudentPage() {
  const {
    activeStudent, activeStudentId, rewards, setActiveStudent, claimPlan, moveTask, startTask,
  ...
  const activeTask = activeStudent.tasks.find((task) => task.id === taskId) ?? null;
  ...
  const firstTodo = activeStudent.tasks.find((task) => !['done', 'skipped'].includes(task.status));
  ...
  const totalFocusMinutes = useMemo(
    () => activeStudent.tasks.filter((task) => task.kind === 'homework').reduce((sum, task) => sum + task.estimatedMinutes, 0),
    [activeStudent.tasks],
  );
  ...
  return (
    <main className="prototype-stage student-stage">
      <aside className="prototype-rail student-rail">
        ...
        <div className="scenario-card">
          ...
          <button onClick={() => { if (!activeStudent.claimed) claimPlan(); setTab('today'); showToast('计划已认领'); }}><Play />进入执行状态</button>
        ...
        <div className="student-app">
          ...
          <div className="tablet-main">
            ...
            <div className="tablet-content">
              {tab === 'today' && !activeStudent.claimed && (
                <section className="claim-view">
                  <div className="claim-hello">
                    <div><span className="student-eyebrow"><Sparkles />嘀嘀已准备好</span><h2>{activeStudent.name.slice(1)}，来认领今天的计划吧！</h2><p>我按照作业量、难度和用眼时间排好了。你可以先调整，再决定自己的节奏。</p></div>
                    <img src="/assets/didi-mascot.png" alt="嘀嘀邀请你认领今日计划" />
                  </div>
                  <div className="claim-summary">
                    <span><strong>{homeworkTasks.length}</strong> 项作业</span><span><strong>{totalFocusMinutes}</strong> 分钟专注</span><span><strong>{activeStudent.plannedEnd}</strong> 预计完成</span><span><strong>55</strong> 分钟晚间时光</span>
                  </div>
                  ...
                  <div className="claim-tasks">
                    {activeStudent.tasks.map((task, index) => (
                      <article key={task.id} className={`claim-task ${task.kind}`}>
                        ...
                        <div className="claim-reorder"><button disabled={index === 0} onClick={() => moveTask(task.id, -1)}><ChevronUp /></button><button disabled={index === activeStudent.tasks.length - 1} onClick={() => moveTask(task.id, 1)}><ChevronDown /></button></div>
                      </article>
                    ))}
                  </div>
              ...
              {tab === 'today' && activeStudent.claimed && (
                <section className="student-today-view">
                  <div className="student-welcome">
                    <div><span>下午好，{activeStudent.name.slice(1)}</span><h2>{done === homeworkTasks.length ? '今天全部完成啦！' : '一步一步，按自己的节奏来'}</h2><p>{done === 0 ? '先完成一个短热身，接着挑战今天最需要专注的任务。' : `已经完成 ${done} 项，嘀嘀一直陪着你。`}</p></div>
                  ...
                  <section className="student-progress-card">
                    ...
                    <div className="progress-details"><span className="student-eyebrow"><Sparkles />嘀嘀的实时预估</span><h3>预计 {activeStudent.plannedEnd} 完成</h3><p>{activeStudent.plannedEnd === activeStudent.originalEnd ? '计划进行得刚刚好，完成后还有 55 分钟晚间时光。' : `原计划 ${activeStudent.originalEnd} 完成，已根据实际进度自动更新。`}</p><div><span>🔥 连胜 {activeStudent.streak} 天</span><span>⭐ 今天 +{done * 5}</span></div></div>
                    <img src="/assets/didi-mascot.png" alt="嘀嘀正在陪伴计划" />
                  </section>

                  <div className="student-section-title"><div><strong>今天的时间线</strong><small>一次只专心做好一件事</small></div><button onClick={() => showToast('长按任务即可拖动调整')}><Move />调整顺序</button></div>
                  <div className="student-timeline">
                    {activeStudent.tasks.map((task) => (
                      <article key={task.id} className={`student-task-card ${task.kind} ${task.status} ${task.id === firstTodo?.id ? 'next' : ''}`}>
                        <div className="student-task-time"><strong>{task.start}</strong><small>{task.end}</small></div>
                        <span className="student-task-emoji">{task.status === 'done' ? <Check /> : task.icon}</span>
                        <div className="student-task-copy"><span>{task.subject} · {task.type}</span><strong>{task.title}</strong><p>{task.estimatedMinutes} 分钟 · {task.difficulty}{task.confidence === 'low' ? ' · 待确认' : ''}</p>{task.needsHelp && <em>已向家长求助，将在最后再试试</em>}</div>
                        <button className="student-task-action" disabled={task.status === 'done'} onClick={() => openTask(task)}>{task.status === 'done' ? '完成' : task.status === 'active' ? '继续' : task.kind === 'break' ? '休息' : '开始'}</button>
                      </article>
                    ))}
                  </div>
                  <button className="student-add-task" onClick={() => showToast('可以手动添加临时任务或学科')}><Plus />我还想添加一项</button>
                </section>
              )}
              ...
              {tab === 'growth' && (
                <section className="growth-view">
                  ...
                  <div className="student-section-title"><div><strong>嘀嘀的衣橱</strong><small>坚持打卡，解锁不同伙伴皮肤</small></div><span className="collection-count">2 / 6</span></div>
                  ...
                  <div className="student-section-title"><div><strong>里程碑徽章</strong><small>看看这周新获得了什么</small></div></div>
              ...
              {tab === 'rewards' && (
                <section className="rewards-view">
                  ...
                  <section className="star-wallet"><div><Star /><span><small>我的星星</small><strong>{activeStudent.stars}</strong></span></div><p>完成任务、认真休息和自主规划都能获得星星。</p><span>本周已获得 36 颗</span></section>
              ...
              {tab === 'report' && (
                <section className="student-report-view">
                  ...
                  <section className="student-week-hero"><div><span>本周自主完成率</span><strong>92%</strong><p>比上周进步了 8%，最明显的进步是“主动调整顺序”。</p></div><img src="/assets/didi-mascot.png" alt="嘀嘀为本周进步庆祝" /><div className="week-stars"><Star />本周 +36</div></section>
                  <div className="student-week-stats"><article><span>🔥</span><p><strong>{activeStudent.streak} 天</strong><small>连续完成</small></p></article><article><span>🎯</span><p><strong>6 次</strong><small>自主调整</small></p></article><article><span>⏱️</span><p><strong>6 分钟</strong><small>平均估时误差</small></p></article></div>
                  <section className="student-chart-card">
                    <div><strong>每天用了多久？</strong><span><i />预估　<i />实际</span></div>
                    <div className="student-bars">{weekChart.map((item) => <div key={item.day}><span><i style={{ height: `${item.estimated * .75}px` }} /><i style={{ height: `${item.actual * .75}px` }} /></span><small>周{item.day}</small></div>)}</div>
                  </section>
      ...
      {toast && <div className="prototype-toast student-toast"><Check />{toast}</div>}
      ...
      <Dialog open={overrunOpen} onOpenChange={setOverrunOpen}>
        <DialogContent className="overrun-dialog" showCloseButton={false}>
          ...
          <div className="time-shift"><span><small>原预计完成</small><strong>{activeStudent.originalEnd}</strong></span><i>→</i><span><small>更新后预计</small><strong>{activeStudent.plannedEnd === activeStudent.originalEnd ? '20:05' : activeStudent.plannedEnd}</strong></span></div>
          <button className="decision-card challenge" onClick={() => { handleOverrun(taskId ?? firstTodo?.id ?? activeStudent.tasks[0].id, 'challenge'); setOverrunOpen(false); showToast('继续挑战！剩余计划已更新'); }}><span>💪</span><p><strong>继续挑战</strong><small>我有信心完成，顺延后面的安排</small></p><ChevronDown /></button>
          <button className="decision-card skip" onClick={() => { handleOverrun(taskId ?? firstTodo?.id ?? activeStudent.tasks[0].id, 'skip'); setOverrunOpen(false); showToast('已移到最后，并通知家长来帮忙'); }}><span>🛟</span><p><strong>暂时跳过</strong><small>先做下一项，把它移到最后并求助家长</small></p><ChevronDown /></button>
          <p className="overrun-note">无论选择哪一个，必要的护眼休息都不会被压缩。</p>
        </DialogContent>
      </Dialog>

      <Dialog open={scheduleOpen} onOpenChange={setScheduleOpen}>
        <DialogContent className="student-schedule-dialog">
          ...
          <div className="student-schedule-list"><div><span>🏠</span><p><small>到家时间</small><strong>17:20</strong></p><button>修改</button></div><div className="special"><span>🏀</span><p><small>兴趣班 · 今天例外</small><strong>20:00–21:00</strong></p><button>修改</button></div><div><span>🌙</span><p><small>准备睡觉</small><strong>21:30</strong></p><button>修改</button></div></div>
          <div className="eye-settings"><Eye /><p><strong>{activeStudent.grade}默认专注 {getFocusMinutes(activeStudent.grade)} 分钟</strong><small>完整休息 10 分钟 · 规则可和家长一起调整</small></p><button>调整</button></div>
      ...
      <Dialog open={celebration} onOpenChange={setCelebration}>
        <DialogContent className="celebration-dialog" showCloseButton={false}>
          ...
          <h2>太棒了，{activeStudent.name.slice(1)}！</h2><p>你完成了自己的全部计划，嘀嘀解锁了今日专属闪光特效。</p>
          <div className="celebration-rewards"><span><Star />+25 星星</span><span><Medal />连胜 {activeStudent.streak + 1} 天</span></div>
