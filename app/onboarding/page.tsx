        ...
        <section className="onboarding-card">
          ...
          {step === 0 && (
            <div className="onboarding-panel identity-panel">
              <span className="panel-icon"><UserRound /></span><h2>你和孩子是什么关系？</h2><p>以后每次调整计划，家人都能看到是谁做出的改变。</p>
          ...
          {step === 1 && (
            <div className="onboarding-panel family-panel">
              <span className="panel-icon"><UsersRound /></span><h2>给家庭空间起个名字</h2><p>家长与孩子会在同一个家庭空间中协作，多位孩子的数据相互隔离。</p>
          ...
          {step === 2 && (
            <div className="onboarding-panel children-panel">
              <span className="panel-icon"><Sparkles /></span><h2>添加需要规划作业的孩子</h2><p>我们准备了两位演示孩子，用来体验多孩切换和独立数据。</p>
              ...
              <button className="add-another-child"><Plus />再添加一个孩子</button>
              <div className="isolation-note"><ShieldCheck /><p><strong>多孩数据隔离</strong><small>每个孩子有独立的任务、用时曲线、星星和奖励记录。</small></p></div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="onboarding-panel eye-panel">
              <span className="panel-icon"><Eye /></span><h2>按年级设置健康用眼节奏</h2><p>这是辅助养成习惯的默认值，家长和孩子以后都可以一起调整。</p>
          ...
          {step === 5 && (
            <div className="onboarding-panel bind-panel">
              {!bound ? (
                <><span className="panel-icon"><QrCode /></span><h2>绑定孩子的平板或学习机</h2><p>在孩子设备打开作业时光，再用当前家长微信扫码完成绑定。</p><div className="large-mock-qr"><QrCode /></div><div className="bind-steps"><span><b>1</b>孩子设备打开绑定页</span><i>→</i><span><b>2</b>家长微信扫码</span><i>→</i><span><b>3</b>选择孩子账号</span></div><button className="simulate-bind" onClick={() => setBound(true)}>模拟扫码绑定林小满</button><small className="demo-qr-note">原型二维码不会连接真实微信</small></>
              ) : (
