...
- 正式业务闭环：家长端录入与排期 → 孩子开始、计时、完成或处理超时 → 剩余计划动态顺延 → 双端同步与调整审计 → 成长趋势与周目标结算 → 徽章、奖励审批、星星账本及嘀嘀皮肤装备。

目前已经完成阶段 0、阶段 2 的最小闭环，阶段 3～4 的执行与动态排期，以及阶段 5 的激励与周报闭环。阶段 1 的真实微信登录仍暂缓，当前继续使用模拟微信身份和模拟 OCR。家庭、孩子、作业、计划、计时、历史耗时、每日成长趋势、周目标、徽章、星星流水、奖励历史和操作记录均由 Spring Boot API 写入 MySQL；上传的作业图片写入 MinIO。Redis 已纳入本地基础设施，留给正式登录会话、缓存和跨实例事件分发。
...
3. 启动孩子端（新终端，在仓库根目录）：
   ...
   `?childId=demo-child-keke` 模拟另一台已绑定的孩子设备。
...
- 不接真实微信登录、OCR/AI 或云对象存储；阶段 1 将在产品闭环稳定后单独实施。
