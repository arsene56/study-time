...
- 正式业务闭环：家长端录入与排期 → 孩子开始、计时、完成或处理超时 → 剩余计划动态顺延 → 双端同步与调整审计 → 周报、徽章、奖励申请及家长审批。

目前已经完成阶段 0、阶段 2 的最小闭环，以及阶段 3～4 的执行与成长闭环。阶段 1 的真实微信登录仍暂缓，当前继续使用模拟微信身份和模拟 OCR。家庭、孩子、作业、计划、计时、历史耗时、周报、星星、奖励和操作记录均由 Spring Boot API 写入 MySQL；上传的作业图片写入 MinIO。Redis 已纳入本地基础设施，留给正式登录会话、缓存和跨实例事件分发。
