...
public class HomeworkService {
    ...
    @Transactional
    public HomeworkBatchView recognize(String studentId, MultipartFile file) {
        ...
        try {
            ...
            List<ParsedTask> parsedTasks = textParser.parse(result, student.grade());
            if (parsedTasks.isEmpty()) {
                ...
                for (int index = 0; index < parsedTasks.size(); index++) {
                    ...
                    insertSeed(batchId, student, new TaskSeed(
    ...
    public HomeworkBatchView addTask(String batchId, SaveHomeworkTaskRequest request) {
        ...
        repository.insertTask(new TaskRow(
                UUID.randomUUID().toString(), batchId, batch.studentId(), request.subject().trim(),
    ...
    public HomeworkBatchView updateTask(String taskId, SaveHomeworkTaskRequest request) {
        ...
        TaskRow updated = new TaskRow(
                current.id(), current.batchId(), current.studentId(), request.subject().trim(),
                request.title().trim(), request.taskType().trim(),
                GradeTaskRules.icon(request.subject().trim(), request.taskType().trim()),
    ...
    public PlanView confirmAndPlan(String batchId, LocalTime startTime) {
        ...
        updates.publish(plan.studentId(), "PLAN_CREATED");
        return plan;
    }
    ...
    private void insertSeed(String batchId, StudentRow student, TaskSeed seed, int index) {
        Estimate estimate = estimationService.estimate(
                student.id(), seed.subject(), seed.taskType(), seed.minutes());
        repository.insertTask(new TaskRow(
                UUID.randomUUID().toString(), batchId, student.id(), seed.subject(), seed.title(),
